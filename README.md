...
`guchat` 是一个使用 Go + Gin 实现的面向多用户的 AI Agent 后端。除用户鉴权、会话与消息、模型配置、流式生成和 SSE 推送外，还支持工具调用（含 MCP）、记忆 RAG、上下文摘要压缩，以及可选的 Docker 终端沙箱。每个会话有独立工作区，文件与终端容器按会话隔离；模型可在对话中读写该工作区，并在隔离容器中执行命令。
